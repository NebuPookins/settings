...
--, className =? "Chromium"                          --> doShift "2:<icon=/home/nebu/.xmonad/fs_01.xbm/>" --problems with save dialog, etc.
  , className =? "Etterna"                           --> doFloatWithSizePixels 1600 900
...
--, title     =? "League of Legends"                 --> doShift "7:games"
  ...
  --TODO: Still figuring out how to make the Google sign in window for Nailbox float. It's possible that Xmonad only sees the initial title, and the title changes after the Google form loads.
  -- , title     =? "Sign in - Google Accounts - Brave" --> doFloat
  , role      =? "pop-up"                            --> doRectFloat (W.RationalRect 0.25 0.25 0.5 0.5)
  ]
  where
